...
def env_fn(env_name, simu_args = {}, reward_func = 'Normalized', reward_args = {}):
...
def create_env(env_name,simu_args={},reward_func='Normalized',reward_args={},state=None,param=None):
    env = gym.make(env_name)
    env.init(simu_args, reward_func, reward_args)
...
def test_agent(env,agent,t_end,x1_ref_seq=None,x3_ref_seq=None,w_seq=None):

    # Start storing
    time = np.arange(0, t_end, env.dt)
    state_record = np.zeros([time.shape[0], 7])
    obs_record = np.zeros([time.shape[0], 9])
    ...
    for i in range(len(time)):
        ...
        state = env.state
        if x1_ref_seq:
            state[4] = x1_ref_seq[i]
        if x3_ref_seq:
            state[5] = x3_ref_seq[i]
        if w_seq:
            state[6] = w_seq[i]
        obs = env.reset(state)
...
def plot_test(state_record, action_record, t_end):
...
def evaluate_control(env,agent,agent_name,t_end,ss_bound):
    ...
    for i in range(num_exp):
        ...
        # Dont forget that u are normalized
